...
export function BanSearch() {
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    try {
      const data = await callApi("ban", { query })
      const formatted = formatApiResponse(data, "ban")
      setResult(formatted)
      playSuccessSound()
    ...
    <div className="space-y-6">
      <form onSubmit={handleSearch} className="flex gap-2">
        <input
          ...
          placeholder="Enter UID to check ban status..."
      ...
      {result && (
        <div ref={resultRef} className="p-6 bg-background border border-border rounded-lg">
          <h3 className="text-lg font-semibold mb-4 text-accent">Ban Status</h3>
