...
export function PostalSearch() {
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    try {
      const data = await callApi("postal", { query })
      const formatted = formatApiResponse(data, "postal")
      setResult(formatted)
      playSuccessSound()
    ...
    <div className="space-y-6">
      ...
      {result && (
        <div ref={resultRef} className="p-6 bg-background border border-border rounded-lg">
          <h3 className="text-lg font-semibold mb-4 text-accent">Postal Information</h3>
